...
const [errorMessage, setErrorMessage] = useState("");
  ...
  const fetchClasses = async () => {
    try {
      ...
      setClasses(res.data.data); // key = data
    } catch (err) {
  ...
  try {
    const res = await axios.get(`http://localhost:3000/api/subject/byclass/${className}`);
  ...
  // When Class Changes → Fetch Subjects
  ...
  try {
    ...
    if (err.response && err.response.status === 400) {
      // duplicate exam message from backend
    ...
    <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            setExam({ ...exam, subjectId: e.target.value })
          }
